...
def _readblock(blocksize: int, bitvector: BitVectorNumPy) -> str:
    ...
    Args:
        blocksize: The requested number of bits to read from the stream. Must
            be a multiple of 8.
        bitvector: The target BitVectorNumPy instance whose FILEIN file stream is
...
class BitVectorNumPy:
    ...
    def __init__(
        ...
        *,
        ...
        """Initializes a BitVectorNumPy instance from one of several possible input sources.
        ...
        if filename is not None:
            ...
            self.vector = np.zeros(eight_byte_ints_needed, dtype=np.uint64)
            return
    ...
    def _getbit(self, pos: int | slice | Any) -> Any:
        ...
        Returns:
            An integer (0 or 1) if pos is an integer index, or a new BitVectorNumPy
        ...
        if not isinstance(pos, slice):
            ...
            if i is None:
                if j >= 0:
                    ...
                    return BitVectorNumPy(bitlist=slicebits)
                else:
                    if abs(j) > len(self):
                        raise ValueError("illegal slice index values")
            ...
            if i >= 0 and j < 0:
                if len(self) - abs(j) < i:
                    ...
                    return BitVectorNumPy(bitlist=slicebits)
            if self.size == 0:
                return BitVectorNumPy(bitstring="")
            if i == j:
                return BitVectorNumPy(bitstring="")
            for x in range(i, j):
                slicebits.append(self[x])
    ...
    def __xor__(self, other: BitVectorNumPy) -> Self:
        ...
        Args:
            other: The second BitVectorNumPy operand.

        Returns:
            A new BitVectorNumPy instance containing the bitwise XOR result.
        """
        ...
        lpb = bv1.vector ^ bv2.vector
        res.vector = np.array(lpb, dtype=np.uint64)
        return res

    def __and__(self, other: BitVectorNumPy) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance containing the bitwise AND result.
        """
        if self.size < other.size:
            bv1 = self._resize_pad_from_left(other.size - self.size)
            bv2 = other
    ...
    def __or__(self, other: BitVectorNumPy) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance containing the bitwise OR result.
        """
        if self.size < other.size:
            bv1 = self._resize_pad_from_left(other.size - self.size)
            bv2 = other
    ...
    def __invert__(self) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance where each 0 bit is replaced by 1 and
            each 1 bit is replaced by 0.
        """
        res = self.__class__(size=self.size)
        res.vector = ~np.array(self.vector, dtype=np.uint64)
        return res

    def __add__(self, other: BitVectorNumPy) -> Self:
        ...
        Args:
            other: The BitVectorNumPy instance to append to the end of this vector.

        Returns:
            A new BitVectorNumPy instance representing the concatenated bit string.
        """
        out_str = str(self) + str(other)
        return self.__class__(bitstring=out_str)

    def __iadd__(self, other: BitVectorNumPy) -> Self:
        """Appends another bit vector to this vector in-place.

        Extends the current bit vector's storage array by appending all bits
        from the argument vector without allocating a new BitVectorNumPy object.

        Args:
            other: The BitVectorNumPy instance to append to this vector.

        Returns:
            This BitVectorNumPy instance (self) after in-place modification.
    ...
    def read_bits_from_file(self, blocksize: int) -> Self:
        ...
        The BitVectorNumPy instance must have been initialized with a filename. Reads up
        ...
        Returns:
            A new BitVectorNumPy instance containing the bits read from the file.
        ...
        """
        error_str = (
            "You need to first construct a BitVectorNumPy "
    ...
    def divide_into_two(self) -> list[Self]:
        ...
        Returns:
            A list of two new BitVectorNumPy instances: [left_half, right_half].
    ...
    def permute(self, permute_list: Sequence[int] | Any) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance containing the permuted bits.
    ...
    def unpermute(self, permute_list: Sequence[int] | Any) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance with bits restored to their unpermuted order.
    ...
    def close_file_object(self) -> None:
        """Closes the input file stream associated with this BitVectorNumPy.
    ...
    def __lshift__(self, n: int) -> Self:
        ...
        Returns:
            This BitVectorNumPy instance (self) after in-place rotation.
    ...
    def circular_rotate_left_by_one(self) -> None:
        ...
        left_most_bits = self.vector & np.uint64(1)
        left_most_bits = np.roll(left_most_bits, -1)
        self.vector = (self.vector >> np.uint64(1)) | (left_most_bits << np.uint64(63))
    ...
    def circular_rotate_right_by_one(self) -> None:
        ...
        right_most_bits = self.vector & np.uint64(0x8000000000000000)
        self.vector = self.vector & np.uint64(0x7FFFFFFFFFFFFFFF)
        right_most_bits = np.roll(right_most_bits, 1)
        right_most_bits[0] = np.uint64(bitstring_rightmost_bit) << np.uint64(63)
        self.vector = (self.vector << np.uint64(1)) | (right_most_bits >> np.uint64(63))
    ...
    def shift_left(self, n: int) -> Self:
        ...
        Returns:
            This BitVectorNumPy instance (self) after in-place shifting.
        """
        for i in range(n):
    ...
    def __setitem__(self, pos: int | slice | Any, item: int | BitVectorNumPy | Any) -> Any:
        ...
        slice assignment (replacing a slice of bits with another BitVectorNumPy).

        Args:
            pos: An integer index or slice object indicating where to assign.
            item: An integer (0 or 1) for index assignment, or a BitVectorNumPy for
                slice assignment.
        ...
        if isinstance(pos, slice):
            if not isinstance(item, BitVectorNumPy):
                raise TypeError(
                    "For slice assignment, the right hand side must be a BitVectorNumPy"
                )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        Returns:
            True if other is a BitVectorNumPy of identical size and bit values,
            or if other is an int/float equal to the integer value of this
            vector. Otherwise False.
        """
        if isinstance(other, BitVectorNumPy):
            if self.size != other.size:
                return False
            for i in range(self.size):
                if self[i] != other[i]:
    ...
    def __lt__(self, other: object) -> bool:
        ...
        Args:
            other: The BitVectorNumPy, int, or float instance to compare against.

        Returns:
            True if this vector's integer value is less than other's.

        Raises:
            TypeError: If other is not a BitVectorNumPy, int, or float.
        """
        if isinstance(other, BitVectorNumPy):
        ...
        raise TypeError(
            f"'<' not supported between instances of 'BitVectorNumPy' and '{type(other).__name__}'"
        )
    ...
    def __deepcopy__(self, memo: dict[int, Any] | None = None) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance identical to this vector.
        """
        if memo is None:
            memo = {}
    ...
    def _resize_pad_from_left(self, n: int) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance containing the left-padded bits.
        """
    ...
    def reset(self, val: int | Any) -> Self:
        ...
        Returns:
            This BitVectorNumPy instance (self) after resetting.
    ...
    def count_bits(self) -> int:
        ...
        for val in self.vector[:-1]:
            total_bits += int(int(val).bit_count())

        if len(self.vector) > 0:
            last_val = self.vector[-1]
            if self.size % 64 != 0:
                # Mask out unused bits in the last uint64
                mask = np.uint64(0xFFFFFFFFFFFFFFFF) >> np.uint64(64 - (self.size % 64))
                last_val = last_val & mask
            total_bits += int(int(last_val).bit_count())
        return total_bits
    ...
    def jaccard_similarity(self, other: BitVectorNumPy) -> float:
        """Calculates the Jaccard similarity coefficient between two vectors.

        Args:
            other: A BitVectorNumPy of equal length to compare against.
    ...
    def jaccard_distance(self, other: BitVectorNumPy) -> float:
    ...
    def hamming_distance(self, other: BitVectorNumPy) -> int:
    ...
    def is_power_of_2(self) -> bool:
        ...
        if self.int_val() == 0:
            return False
        bv = self & BitVectorNumPy(intVal=self.int_val() - 1)
        if bv.int_val() == 0:
            return True
        return False
    ...
    def reverse(self) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance with bits in reversed order.
        """
        reverseList = []
        i = 1
        while i < self.size + 1:
    ...
    def gcd(self, other: BitVectorNumPy) -> Self:
        """Calculates the greatest common divisor (GCD) using Euclid's algorithm.

        Args:
            other: A BitVectorNumPy representing the second integer operand.

        Returns:
            A new BitVectorNumPy instance containing the GCD of the two integer values.
        """
        a = self.int_val()
        b = other.int_val()
        if a < b:
    ...
    def multiplicative_inverse(self, modulus: BitVectorNumPy) -> Self | None:
        ...
        Args:
            modulus: A BitVectorNumPy representing the integer modulus.

        Returns:
            A new BitVectorNumPy with the multiplicative inverse modulo modulus,
    ...
    def gf_multiply(self, b: BitVectorNumPy) -> Self:
        """Multiplies two polynomials in Galois Field GF(2).

        Args:
            b: The second polynomial BitVectorNumPy operand.

        Returns:
            A new BitVectorNumPy containing the GF(2) product of the two polynomials.
        """
    ...
    def gf_divide_by_modulus(self, mod: BitVectorNumPy, n: int) -> tuple[Self, Self]:
        """Divides this polynomial by a modulus polynomial in GF(2^n).

        Args:
            mod: A BitVectorNumPy representing the modulus polynomial.
            n: The integer degree n of the Galois Field GF(2^n).

        Returns:
            A tuple of two BitVectorNumPys: (quotient, remainder).
    ...
    def gf_multiply_modular(self, b: BitVectorNumPy | Any, mod: BitVectorNumPy, n: int) -> Self:
        ...
        Args:
            b: The second polynomial operand BitVectorNumPy.
            mod: The modulus polynomial BitVectorNumPy.
            n: The integer degree n of the Galois Field GF(2^n).

        Returns:
            A new BitVectorNumPy containing the product modulo mod in GF(2^n).
        """
        a = self
        a_copy = copy.deepcopy(a)
        b_copy = copy.deepcopy(b)
    ...
    def gf_MI(self, mod: BitVectorNumPy, n: int) -> Self | tuple[str, ...]:
        ...
        Returns:
            A new BitVectorNumPy with the multiplicative inverse in GF(2^n), or a
            tuple of descriptive strings if no inverse exists.
        """
        num: BitVectorNumPy = self
    ...
    def gen_random_bits(self, width: int) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance containing the generated random bits.
        """
    ...
    def min_canonical(self) -> Self:
        ...
        Returns:
            A new BitVectorNumPy instance representing the minimum canonical rotation.
        """
